...
export function WorkspaceShell({ onAddStore, onLogout }: WorkspaceShellProps) {
    ...
    if (!bootstrapped) {
        return (
            <div className="flex min-h-screen w-full items-center justify-center bg-[#f5f2eb]">
                <span className="flex h-12 w-12 animate-pulse items-center justify-center rounded-2xl bg-[#172b4d] text-[#3dd7af]">
                    <StoreIcon size={22} />
                </span>
            </div>
        )
            ...
            <div className="flex min-h-screen w-full items-center justify-center bg-[#f5f2eb] p-5">
                <section className="max-w-md rounded-3xl border border-[#ded9cf] bg-white p-8 text-center shadow-sm">
                    ...
                    <p className="mt-2 text-sm leading-6 text-slate-500">
                        가게 정보가 있어야 콘텐츠 초안과 운영 화면을 준비할 수
                        있어요.
                    </p>
                    <button
                        type="button"
